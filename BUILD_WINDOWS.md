...
With the env loaded (`. .\setup-env.ps1`), `build-module.ps1 <module> [-Target <t>] [-Jobs N]`
configures + builds + installs one module (`-Jobs` defaults to 4). The full sequence:
...
`-Jobs N` sets the parallel compile jobs forwarded to every module (default 4).
